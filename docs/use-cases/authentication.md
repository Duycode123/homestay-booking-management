...
Let a user sign in with email/password, Google, or Facebook and establish exactly one trusted browser identity. The UI, role-based navigation, and protected APIs must all describe the same authenticated account.

## Actors

- Customer, staff member, or administrator
- Google OAuth 2.0 or Facebook Login
...
- An OAuth provider returns an email and a provider subject; the account is linked or created by the OAuth use case.
- Production uses HTTPS and `APP_COOKIE_SECURE=true`.

## Main flow

1. The user signs in with credentials or starts Google or Facebook OAuth.
...
- OAuth failure clears auth cookies and redirects to `/login?oauthError=oauth_login_failed`.
...
- HttpOnly cookies use `SameSite=Lax` for the top-level OAuth redirect and `Secure` in production.
...
- OAuth handlers are inbound adapters and delegate account linking to `AuthenticateOAuthUserUseCase`; Google and Facebook identities are stored with the same provider-agnostic port.

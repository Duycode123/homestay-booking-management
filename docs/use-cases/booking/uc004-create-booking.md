...
13. If the customer leaves that page before payment, frontend sends an unload-safe request to the dedicated page-exit release endpoint. It does not depend on the login cookie or CSRF header because either may disappear during navigation or account switching. The random `PAY...` reference acts as the short-lived release capability; the endpoint is idempotent and can only expire a still-pending transaction. The transaction and still-pending booking become `EXPIRED`; the expiry sweep remains the fallback when the browser cannot deliver either beacon or keepalive fetch.

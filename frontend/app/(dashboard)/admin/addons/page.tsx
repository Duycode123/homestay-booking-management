        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-inverse-surface/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="addon-form-title">
          ...
          <section className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-white/30 bg-white shadow-[var(--shadow-elevated)]">
            ...
            <div className="grid gap-5 p-6 sm:grid-cols-2">
              ...
              <Field label="Ảnh minh họa (không bắt buộc)" className="sm:col-span-2"><input value={form.imageUrl ?? ''} maxLength={500} onChange={(event) => setForm({ ...form, imageUrl: event.target.value })} placeholder="https://... hoặc /images/..." className={inputClass} /></Field>
            </div>
...
function ServiceCard({ item, onEdit, onToggle }: { item: AddonCatalogItem; onEdit: (item: AddonCatalogItem) => void; onToggle: () => void }) {
  return <article className={`group flex min-h-56 flex-col rounded-2xl border p-5 transition ${item.active ? 'border-outline-variant bg-white hover:-translate-y-0.5 hover:border-brand-orange/35 hover:shadow-[var(--shadow-card)]' : 'border-outline-variant bg-surface-container-low opacity-75'}`}>

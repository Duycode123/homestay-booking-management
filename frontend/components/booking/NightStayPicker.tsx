...
export default function NightStayPicker({ roomId, initialDate, initialEndDate, onChange, className = '' }: NightStayPickerProps) {
  ...
  return (
    <section className={['overflow-hidden rounded-[26px] border border-[#dfd3c3] bg-[#fffdf9] shadow-[0_22px_70px_rgba(30,48,40,.10)]', className].join(' ')}>
      <div className="border-b border-[#e8dfd2] bg-[linear-gradient(135deg,#173a31,#254f43)] px-5 py-5 text-white sm:px-6">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {[1, 2, 3, 4, 7].map((count) => <button key={count} type="button" onClick={() => setCheckOutDate(addDays(checkInDate, count))} className={['rounded-full border px-4 py-2 text-xs font-bold transition', nights === count ? 'border-[#173a31] bg-[#173a31] text-white' : 'border-[#ddd2c3] bg-white text-[#5e625f] hover:border-[#b28455]'].join(' ')}>{count} đêm</button>)}
        </div>
...
function CalendarPanel({ activePicker, visibleMonth, checkInDate, checkOutDate, minDate, maxDate, onPrevious, onNext, onSelect, onClose }: { activePicker: Exclude<ActiveDatePicker, null>; visibleMonth: string; checkInDate: string; checkOutDate: string; minDate: string; maxDate: string; onPrevious: () => void; onNext: () => void; onSelect: (date: string) => void; onClose: () => void }) {
  ...
  return <div className="mt-4 overflow-hidden rounded-[24px] border border-[#ddd0bf] bg-white shadow-[0_24px_64px_rgba(31,48,41,.13)]"><div className="flex items-center justify-between border-b border-[#eee6db] bg-[#fbf8f3] px-4 py-3 sm:px-5"><button type="button" onClick={onPrevious} className="flex h-10 w-10 items-center justify-center rounded-full border border-[#ded3c4] bg-white text-xl text-[#3d4b45] transition hover:border-[#b28455] hover:text-[#9b6b3c]" aria-label="Tháng trước">‹</button><div className="text-center"><p className="text-xs font-bold uppercase tracking-[0.15em] text-[#9b6b3c]">{activePicker === 'checkin' ? 'Chọn ngày nhận phòng' : 'Chọn ngày trả phòng'}</p><p className="mt-0.5 text-xs text-[#7a7d77]">Tối đa 30 đêm cho mỗi lượt đặt</p></div><button type="button" onClick={onNext} className="flex h-10 w-10 items-center justify-center rounded-full border border-[#ded3c4] bg-white text-xl text-[#3d4b45] transition hover:border-[#b28455] hover:text-[#9b6b3c]" aria-label="Tháng sau">›</button></div><div className="grid gap-7 p-4 sm:p-5 md:grid-cols-2"><CalendarMonth month={visibleMonth} checkInDate={checkInDate} checkOutDate={checkOutDate} minDate={minDate} maxDate={maxDate} onSelect={onSelect} /><div className="hidden md:block"><CalendarMonth month={secondMonth} checkInDate={checkInDate} checkOutDate={checkOutDate} minDate={minDate} maxDate={maxDate} onSelect={onSelect} /></div></div><div className="flex items-center justify-between border-t border-[#eee6db] bg-[#fbf8f3] px-5 py-3"><div className="flex items-center gap-4 text-[11px] font-semibold text-[#747871]"><span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-[#173a31]" />Ngày đã chọn</span><span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-[#eadbc8]" />Kỳ lưu trú</span></div><button type="button" onClick={onClose} className="text-xs font-bold text-[#9b6b3c] hover:underline">Đóng lịch</button></div></div>
}
...
function CalendarMonth({ month, checkInDate, checkOutDate, minDate, maxDate, onSelect }: { month: string; checkInDate: string; checkOutDate: string; minDate: string; maxDate: string; onSelect: (date: string) => void }) {
  ...
  return <div><h3 className="text-center font-display text-base font-bold capitalize text-[#26352f]">{formatMonthYear(month)}</h3><div className="mt-4 grid grid-cols-7 text-center text-[10px] font-bold uppercase text-[#999991]">{['T2','T3','T4','T5','T6','T7','CN'].map((day) => <span key={day} className="py-1">{day}</span>)}</div><div className="mt-1 grid grid-cols-7 gap-y-1">{cells.map((date, index) => {
    ...
    return <button key={date} type="button" disabled={disabled} onClick={() => onSelect(date)} className={['relative flex h-10 items-center justify-center text-sm font-semibold transition', endpoint ? 'z-10 rounded-full bg-[#173a31] text-white shadow-[0_7px_18px_rgba(23,58,49,.25)]' : inRange ? 'bg-[#f0e3d2] text-[#654b31]' : 'rounded-full text-[#34413c] hover:bg-[#f5eee5]', disabled ? 'cursor-not-allowed text-[#cdcac3] hover:bg-transparent' : ''].join(' ')} aria-label={formatFullDate(date)}>{Number(date.slice(-2))}</button>
  })}</div></div>
}

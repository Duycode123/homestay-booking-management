...
export default function NewCustomerOfferModal() {
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-[#102D26]/70 px-4 py-6 backdrop-blur-[5px]"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      ...
      >
        <button
          ...
          className="absolute right-4 top-4 z-20 grid h-10 w-10 place-items-center rounded-full border border-white/55 bg-white/85 text-[#173A31] shadow-sm backdrop-blur transition hover:rotate-90 hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C18B52]"
        >
        ...
        <div className="relative min-h-[235px] overflow-hidden md:min-h-[500px]">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#102D26]/70 via-transparent to-transparent md:bg-gradient-to-r md:from-transparent md:to-[#102D26]/15" />
          <div className="absolute bottom-5 left-5 rounded-full border border-white/35 bg-[#102D26]/72 px-4 py-2 text-[11px] font-bold uppercase tracking-[.2em] text-white backdrop-blur">
            Ưu đãi chào đón
          </div>
        </div>
        ...
        <div className="relative flex flex-col justify-center px-6 py-8 sm:px-10 sm:py-10 md:px-12">
          ...
          <div className="mt-5 flex items-end gap-3">
            <span className="font-editorial text-[5.2rem] font-semibold leading-[.8] tracking-[-.06em] text-[#173A31] sm:text-[6rem]">
              {offer.discountPercent}%
            </span>
            <span className="pb-1.5 text-sm font-bold uppercase tracking-[.18em] text-[#B47D46]">giảm ngay</span>
          </div>

          <h2 id="new-customer-offer-title" className="mt-7 font-editorial text-3xl font-semibold leading-tight text-[#173A31] sm:text-[2.15rem]">
          ...
          <div className="mt-6 flex items-center justify-between rounded-2xl border border-dashed border-[#B47D46]/45 bg-white px-4 py-3">
            <div>
              ...
              <p className="mt-0.5 text-lg font-extrabold tracking-[.12em] text-[#173A31]">{offer.code}</p>
            </div>
            <span className="rounded-full bg-[#E9F3EE] px-3 py-1.5 text-xs font-bold text-[#245545]">Booking đầu tiên</span>
          </div>

          <button
            type="button"
            onClick={exploreRooms}
            className="mt-6 inline-flex h-[52px] items-center justify-center gap-2 rounded-full bg-[#173A31] px-6 font-display text-sm font-bold text-white shadow-[0_16px_34px_rgba(23,58,49,.23)] transition hover:-translate-y-0.5 hover:bg-[#245545] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C18B52] focus-visible:ring-offset-2"
          >
            Chọn phòng ngay

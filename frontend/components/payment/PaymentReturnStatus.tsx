...
export default function PaymentReturnStatus() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#F4F0E8] px-4 py-5 text-[#202723] sm:px-6 sm:py-8 lg:px-8">
      ...
      <div aria-hidden className="pointer-events-none absolute -right-24 bottom-0 h-96 w-96 rounded-full bg-[#234D42]/10 blur-3xl" />
      ...
      <div className="relative mx-auto max-w-6xl">
        <header className="flex items-center justify-between gap-4 py-2">
          <Link href="/" className="inline-flex items-center gap-3 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#B98A55]/40">
            <span className="flex h-11 w-11 items-center justify-center rounded-full bg-[#234D42] text-white shadow-[0_10px_24px_rgba(21,63,52,0.18)]">
              <HomeIcon />
            </span>
            <span>
              <span className="block font-display text-lg font-bold leading-tight text-[#234D42]">The Serene Villa</span>
            ...
            <div className={["relative flex min-h-[480px] flex-col justify-between overflow-hidden p-7 sm:p-10 lg:p-12", presentation.panelClass].join(' ')}>
              ...
              <div className="relative mt-10 flex flex-col gap-3 sm:flex-row">
                {!missingBooking && (
                  <Link
                    href={primaryHref}
                    className="inline-flex min-h-13 flex-1 items-center justify-center gap-2 rounded-2xl bg-[#234D42] px-6 py-3.5 font-display font-bold text-white shadow-[0_14px_28px_rgba(24,63,53,0.22)] transition hover:-translate-y-0.5 hover:bg-[#173B32]"
                  >
                    {primaryLabel}
                    <ArrowRightIcon />
                  </Link>
            ...
            <aside className="flex flex-col justify-center bg-white p-7 sm:p-10 lg:p-12" aria-label="Chi tiết giao dịch">
              ...
              <div className="mt-6 rounded-[22px] border border-[#DCE8E2] bg-[#F1F7F4] p-5">
                <div className="flex gap-3">
                  <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#DDECE5] text-[#234D42]">
                    <ShieldIcon />
                  </span>
                  <div>
                    <p className="font-display text-sm font-bold text-[#234D42]">Thông tin của bạn được bảo vệ</p>
...
function getStatusPresentation(status: DisplayStatus, paymentOption: 'full' | 'deposit', missingBooking: boolean) {
  ...
  if (status === 'success') {
    ...
    return {
      ...
      panelClass: 'bg-[#EAF3EE] text-[#234D42]',
      iconClass: 'bg-[#234D42] text-white',

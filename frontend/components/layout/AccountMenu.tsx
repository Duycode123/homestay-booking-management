  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      {open ? (
        ...
        >
          <div className="relative border-b border-white/10 bg-[linear-gradient(145deg,#234D42,#52766B)] px-4 py-4 text-white">
  ...
  return (
    <Link
      ...
      className={[
        ...
        emphasized
          ? 'border-transparent bg-[#f7efe4] text-[#234D42] hover:bg-[#f1e3d1]'
          : 'border-transparent text-[#303833] hover:border-[#e2d5c3] hover:bg-[#f8f3ec]',
      ].join(' ')}
    >
      <span className="flex items-center gap-3">
        <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${emphasized ? 'bg-[#234D42] text-white' : 'bg-[#efe5d7] text-[#74593b]'}`}>
          <Icon name={icon} />
        </span>
...
function AccountAvatar({ avatarUrl, initial, size, alt = 'Ảnh đại diện' }: { avatarUrl?: string; initial: string; size: 'trigger' | 'menu'; alt?: string }) {
  ...
  return (
    <span className={`flex shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#234D42] font-display font-bold text-white ${classes}`}>

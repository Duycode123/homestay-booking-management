...
CREATE TABLE IF NOT EXISTS booking_refund (
    ...
    method VARCHAR(40) NOT NULL CHECK (method IN ('ORIGINAL_PAYMENT_METHOD', 'MANUAL_BANK_TRANSFER', 'CASH_COUNTER')),
    status VARCHAR(30) NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'RETRY_REQUIRED')),
    ...
    CONSTRAINT ck_booking_refund_completed CHECK (
        status <> 'COMPLETED' OR (completed_at IS NOT NULL AND transaction_reference IS NOT NULL)
    )
);

...
- Payment and booking timeout behavior must stay aligned with booking-expiry logic in the backend. Browsing and confirming booking details do not create a database booking or block availability. The booking and payment transaction are created only when the customer requests a VietQR payment session. Each checkout session expires `app.booking.payment-expiration-seconds` seconds after that transaction's `created_at` (default `300`, or 5 minutes). Timeout, session replacement, or leaving the dedicated payment page changes the unpaid transaction and its still-pending booking to `EXPIRED`; this releases the room without recording a customer cancellation. The customer payment-status endpoint applies this timeout, while the scheduled sweep runs every 10 seconds by default to release abandoned holds promptly.
...
- `booking_status.EXPIRED` means an unpaid temporary payment hold was released. It is excluded from room-overlap blocking, customer booking history, admin booking/cancellation lists, staff schedules, and cancellation/refund reporting. `CANCELLED` remains reserved for a real booking cancellation after a confirmed booking exists.
- `payment_transaction_status.EXPIRED` means a pending QR session timed out, was replaced, or was explicitly released when the customer left the payment page. It is operational evidence, not a cancelled stay.
- The no-overlap exclusion constraint ignores both `CANCELLED` and `EXPIRED` bookings. Confirmed paid/deposit bookings continue to block availability until their normal lifecycle releases the room.
...
- `room.max_people` stores the maximum number of people a bookable homestay can hold. `room.image_url` is the primary image; `image_url_2` through `image_url_4` form the detail-page gallery. `common_amenity` stores reusable included-amenity definitions, while `room_included_amenity` assigns each definition to the homestays where it is actually available. The existing equipment tables remain specific to each homestay.
  - `common_amenity.active` controls public visibility. `GET /api/rooms/common-amenities?roomId={roomId}` returns only active amenities assigned to that homestay.
  - `room_included_amenity` has a composite primary key `(room_id, amenity_id)`. Both foreign keys use `ON DELETE CASCADE` so deleting a homestay or amenity cannot leave stale assignments.
  - Migration `V17__scope_included_amenities_by_homestay.sql` backfills existing definitions across existing homestays once to preserve the former global behavior; administrators then maintain each assignment explicitly.

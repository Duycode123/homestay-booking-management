...
CREATE TABLE IF NOT EXISTS customer (
  ...
  CONSTRAINT chk_staff_hourly_rate_non_negative CHECK (hourly_rate >= 0)
);
...
CREATE TABLE IF NOT EXISTS room (
  ...
  check_in_radius_m INT NOT NULL DEFAULT 100,
  ...
  CONSTRAINT chk_room_bedroom_count_range CHECK (bedroom_count BETWEEN 1 AND 20),
  ...
  CONSTRAINT chk_room_longitude_range CHECK (longitude IS NULL OR longitude BETWEEN -180 AND 180),
  CONSTRAINT chk_room_check_in_radius_range CHECK (check_in_radius_m BETWEEN 20 AND 1000),
  CONSTRAINT chk_room_base_nightly_rate_positive CHECK (base_nightly_rate > 0),
...
CREATE TABLE IF NOT EXISTS payment_transaction (
  ...
  processed_by_user_id INT REFERENCES account(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS staff_attendance (
  ...
  CONSTRAINT chk_attendance_check_in_coordinate_pair CHECK (
...
CREATE INDEX IF NOT EXISTS idx_room_district_status ON room(district, status);
CREATE INDEX IF NOT EXISTS idx_room_coordinates
  ON room(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_shift_room_date ON "shift"(room_id, date, start_time);
...
COMMENT ON TABLE room IS 'Each row is one independently bookable whole accommodation at one physical location. Bedrooms are descriptive and are never booked separately.';
COMMENT ON COLUMN room.base_nightly_rate IS 'Whole-unit price for one calendar night, from 14:00 check-in to 12:00 check-out the next day.';
COMMENT ON COLUMN room.check_in_radius_m IS 'Allowed GPS distance from the accommodation marker for staff attendance.';
COMMENT ON COLUMN "shift".room_id IS 'The whole accommodation where this shift is performed.';
COMMENT ON COLUMN staff_attendance.check_in_distance_m IS 'Server-calculated Haversine distance from submitted GPS coordinates to the assigned accommodation.';
